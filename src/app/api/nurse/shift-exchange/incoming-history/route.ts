...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get all shift exchange requests sent to this user that are approved or rejected
    const { data: requests, error } = await supabaseAdmin
      ...
      .select(`
        ...
        reason,
        status,
        requester:users!shift_exchange_requests_requester_id_fkey(
          name,
          email
        )
      `)
    ...
    if (error) {
      console.error('Error fetching incoming history:', error)
